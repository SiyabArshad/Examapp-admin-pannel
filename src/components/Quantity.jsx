...
const Quantity = () => {
    ...
    const[ftpacq,setftpacq]=React.useState(1)
...
const getudatedreq=async()=>{
    setloading(true)
        const upDocRef = doc(db, "quantity", `${process.env.REACT_APP_QUANTITY}`);
        const docSnap = await getDoc(upDocRef);  
  if (docSnap.exists()) {
    setftpq(docSnap.data().ftp)
    setftpacq(docSnap.data().ftpac)
    setloading(false)
  } else {
    setstatuserr(false)
    setmessage(true)
    ...
    <Grid container spacing={3}>
      <Grid item xs={12} sm={6}>
        <TextField
          ...
          type="number"
          fullWidth
          autoComplete="Enter Question Quantity FTP"
          variant="standard"
      ...
      <Grid item xs={12} sm={6}>
        <TextField
          ...
          type="number"
          InputProps={{ inputProps: { min: 1, max: 50 } }}
          autoComplete="Enter Question Quantity FTPAC"
          variant="standard"
          onChange={(e)=>setftpacq(e.target.value)}
        />
      </Grid>
      
      <Grid item xs={12} sm={6}>
      <Button disabled={clicked} onClick={updatequantity}  variant="contained" style={{backgroundColor:"#4169e1"}}>
                  Update
      </Button>
      </Grid>
      <Grid  xs={12}>
      <Typography style={{padding:"15px 0px 0px 15px"}} >
      Number of questions for FTP is <span style={{color:"red"}}>{ftpq}</span>
    </Typography>
    <Typography style={{padding:"15px 0px 0px 15px"}}>
      Number of questions for FTPAC is <span style={{color:"red"}}>{ftpacq}</span>
    </Typography>
    </Grid>
    </Grid>
